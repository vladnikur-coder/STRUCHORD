...
w.addEventListener('load', async () => {
  ...
  console.log('=== 11. Свинг НЕ влияет на отрисовку (граница B-56) ===');
  {
    ...
    // Считать вызовы бессмысленно: их число меняется от рефакторинга
    // (B-63 добавил второй вызов в планировщике — для точки отсчёта
    // абсолютной пары). Важно НЕ количество, а МЕСТО: свинг обязан
    // жить только в звуковых функциях.
    const soundFns = ['schedulePatternForEvent', 'startPatternPreview'];
    const drawFns = ['renderRhythmTrack', 'buildRhythmHintContent',
      'buildInnerCounts', 'buildStrumPreviewEls', 'layoutRhythmHints',
      'syncRhythmHintCounts', 'rhythmHintPatternForSpan'];
    const cut = (name) => {
    ...
    drawFns.forEach((fn) => {
      ok(`${fn} не знает о свинге`, !cut(fn).includes('swingStepOffsetUnits'));
    });
    ok('свинг живёт в звуковых функциях',
       soundFns.every((fn) => cut(fn).includes('swingStepOffsetUnits')));
  }

  console.log('=== 12. Свинг считается от НАЧАЛА КВАДРАТА (B-63) ===');
  {
    // Спека пользователя 2026-09-06: «свинг сбивается из-за того, что
    // начинает считаться заново после ресайза: то, что должно быть "и",
    // становится "1"».
    //
    // Кач определён на ПАРАХ восьмых. Какая из двух нот перед нами,
    // решает абсолютная позиция в квадрате, а не номер шага внутри
    // ячейки. Ячейка, начавшаяся с доли 1.5, открывается «и» — второй
    // нотой пары, и звучать должна на 1.667, а не на 1.5.
    const at = (node) => w.eval(`+swingStepOffsetUnits(${node}, 2, true).toFixed(3)`);
    ok('узел 0 («раз») звучит на 0', at(0) === 0, String(at(0)));
    ok('узел 1 («и») звучит на 0.667', Math.abs(at(1) - 0.667) < 0.001, String(at(1)));
    ok('узел 2 («два») звучит на 1', at(2) === 1, String(at(2)));
    ok('узел 3 («и») звучит на 1.667', Math.abs(at(3) - 1.667) < 0.001, String(at(3)));
    // Ключевое: нечётный узел НИКОГДА не становится сильной долей.
    const odd = [1, 3, 5, 7, 9].map(at);
    ok('все нечётные узлы качнуты (дробная часть 0.667)',
       odd.every((t) => Math.abs((t - Math.floor(t)) - 2 / 3) < 0.001), odd.join(' '));
    const even = [0, 2, 4, 6, 8].map(at);
    ok('все чётные узлы на целых долях',
       even.every((t) => Math.abs(t - Math.round(t)) < 1e-9), even.join(' '));
  }

  console.log('=== 13. Ячейка с дробным началом: «и» остаётся «и» (B-63) ===');
  {
    // Репро пользователя (файл «Дешевые Драмы.struchord-3.json»): F=1.5,
    // поэтому E начинается с доли 1.5 — со ВТОРОЙ ноты пары. Планировщик
    // обязан сыграть её на 1.667, а не прижать к 1.5.
    const song2 = { name: 'swing-frac', bpm: 120, globalKey: 'C', globalTimeSig: '4/4',
      sections: [{ id: 's1', name: 'A', timeSig: '4/4',
        strumPattern: { mode: 'strum', subdivision: 2, swing: true,
          steps: ['D', null, 'D', 'U', null, 'U', 'D', 'U'] },
        squares: [{ id: 'q1', events: [
          { chord: 'F', span: 1.5 }, { chord: 'E', span: 2.5 },
        ] }] }] };
    w.localStorage.setItem('struchord_songs', JSON.stringify([song2]));
    w.loadSong(0);
    const first = w.eval(`(function(){
      const log=[];
      const orig=window.scheduleStrumStepHighlight;
      window.scheduleStrumStepHighlight=function(c,e,i,t){ log.push(t); };
      const sec=sections[0], sq=sec.squares[0], ei=1, ev=sq.events[ei];
      const p=rhythmSoundingForEvent(sec,sq,ev,ei);
      const ts=getEffectiveTimeSigForEvent(ev,sec);
      const unit=getGridUnitDurationSeconds(120,ts);
      try{ schedulePatternForEvent('E',0,10,null,p,120,ts,sq,ei,ev.span,true,sec); }
      finally { window.scheduleStrumStepHighlight=orig; }
      const beats=[...new Set(log)].sort((a,b)=>a-b).map(t=>+(t/unit).toFixed(3));
      return JSON.stringify(beats);
    })()`);
    const beats = JSON.parse(first);
    console.log('      шаги от начала ячейки:', beats.join(' '));
    // Ячейка стоит на 1.5; её первый удар — «и» пары, абсолютно 1.667,
    // то есть 0.167 от начала ячейки.
    ok('первый удар качнут, а не прижат к началу ячейки',
       beats.length > 0 && Math.abs(beats[0] - 0.167) < 0.01, beats.join(' '));
    ok('первый удар НЕ стоит в нуле (это была бы «раз»)',
       beats.length > 0 && beats[0] > 0.05, String(beats[0]));
  }
