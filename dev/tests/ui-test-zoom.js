...
w.addEventListener('load',()=>{
  ...
  // B-71 (0.219): узел берётся, когда курсор до него ДОШЁЛ, без опережения.
  // Квадрат 800px на 16 единиц = 50px/единицу: −13px ставит курсор на
  // 3.74 — ЗА узлом 3.75, граница переходит на четверть. Старое −12px
  // останавливало курсор на 3.76, не дотянув 0.01, — и правильно не
  // меняла ничего.
  const r16=drag(2.5,-13);
  console.log('      ×2.5, тянем -13px:', JSON.stringify(r16));
