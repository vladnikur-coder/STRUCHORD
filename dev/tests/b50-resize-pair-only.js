...
(async () => {
  ...
  //    ударов, сколько будет после отпускания, а не сколько было до.
  {
    ...
    await sleep(120);
    const during = hits();
    // B-61: контракт ИЗМЕНЁН пользователем (2026-09-06): «ритм визуально
    // и аудиально не меняется никогда при ресайзе, меняются только
    // аккорды, поверх которых он играется». Прежний live_count требовал
    // обратного — пересчёта числа ударов под будущие доли прямо в
    // жесте; он отменён. Теперь лента в протяжке НЕПОДВИЖНА.
    ok('в жесте лента ритма не изменилась', during.join(',') === atDown.join(','),
       atDown.join(',') + ' -> ' + during.join(','));
    ok('чужие ячейки тоже не тронуты', during[0] === atDown[0] && during[4] === atDown[4], during.join(','));
