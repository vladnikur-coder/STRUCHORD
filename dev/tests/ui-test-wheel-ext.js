...
w.addEventListener('load', () => {
  ...
  console.log('\n=== 11. B-40: открытие колеса восстанавливает расширение текущего аккорда ===');
  const listBefore = row1() + ' / ' + row2();
  w.eval("activeChordInput = document.querySelector('.chord-input'); openChordWheel(activeChordInput);");
  // Последним введён Eb9; его «9» живёт в очереди, поэтому B-40 открывает
  // точный режим вместо прежнего безусловного сброса в triads.
  ok('режим восстановлен из Eb9', w.eval('wheelMode') === '9', w.eval('wheelMode'));
