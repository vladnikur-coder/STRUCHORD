...
(async()=>{
  ...
  if(!w.eval("audioDiagnosticLog.some(e=>e.type==='context-idle-suspend-resolved')"))throw new Error('ПРОВАЛ: idle suspend не записан в журнал');
  ...
  w.document.dispatchEvent(new w.Event('visibilitychange'));
  await new Promise(resolve=>setTimeout(resolve,0));
  if(first.state!=='running')throw new Error('ПРОВАЛ: активное воспроизведение было усыплено');
