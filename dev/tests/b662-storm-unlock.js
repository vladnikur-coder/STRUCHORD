...
ok(!src.includes('fadeStart =')&&!src.includes('linearRampToValueAtTime(.0001, endAt)')&&!src.includes('exponentialRampToValueAtTime(.0001, ctx.currentTime + Math.min(5.8'),'старое программное затухание mp3 удалено полностью');
const thunderChimeBody = (/function playThunderAchievementChime\(\) \{([\s\S]*?)\n\}/.exec(src)||[])[1] || '';
ok(thunderChimeBody.includes('playThunderAchievementMp3();')&&!thunderChimeBody.includes('getAudioContext')&&!thunderChimeBody.includes('createBuffer')&&!thunderChimeBody.includes('gain.gain'),'Грохочет гром больше не накладывает WebAudio-эффект поверх mp3');
ok(src.includes("@font-face")&&src.includes("./uploads/ofont.ru_Izhitsa.ttf")&&src.includes("font-family:'Izhitsa'")&&src.includes('thunder-plate-main'),'production-плашка использует локальный Izhitsa');
ok(!src.includes('thunder-title-achievement')&&!src.includes('thunder-title-backdrop')&&!src.includes('previewThunderTitleForDev')&&!src.includes('Превью титра'),'старый широкий титр полностью удалён из активного кода и dev-палитры');
ok(src.includes('thunder-plate-achievement')&&src.includes('thunder-plate-etching')&&src.includes('thunder-plate-mark')&&src.includes('thunder-plate-mark-bolt')&&src.includes('previewThunderPlateForDev')&&src.includes('Превью плашки'),'production/dev woodcut-плашка содержит видимую живую молнию');
