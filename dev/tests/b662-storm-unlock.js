...
ok(src.includes('fadeStart = Math.min(endAt - .02, Math.max(attackEnd + .08, endAt - fadeDuration))')&&src.includes('gain.gain.setValueAtTime(.72, fadeStart)')&&src.includes('linearRampToValueAtTime(.0001, endAt)'),'mp3 Грохочет гром держит громкость до хвоста и затухает только в конце');
ok(!src.includes('exponentialRampToValueAtTime(.0001, ctx.currentTime + Math.min(5.8'),'старое мгновенное затухание mp3 от старта удалено');
