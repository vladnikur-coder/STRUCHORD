...
// C-shape barre Eb: x, 6, 5, 3, 3, 3 -> мини-баррэ 3 лад (1), 4-я струна 5 лад (3), 5-я струна 6 лад (4)
const fEb = w.computeFingersForShape(['x', 6, 5, 3, 3, 3]);
check('C-образная форма Eb (x,6,5,3,3,3): пальцы x, 4, 3, 1, 1, 1', fEb, ['x', 4, 3, 1, 1, 1]);

// A-shape barre Bb: x, 1, 3, 3, 3, x -> 5-я струна 1 лад (палец 1), 4/3/2 струны мини-баррэ 3 лад (палец 3)
const fBb = w.computeFingersForShape(['x', 1, 3, 3, 3, 'x']);
check('A-образная форма Bb (x,1,3,3,3,x): баррэ 3 пальцем', fBb, ['x', 1, 3, 3, 3, 'x']);

// A-shape full barre Bb: x, 1, 3, 3, 3, 1 -> баррэ 1 лад (палец 1), 4/3/2 струны 3 лад (пальцы 2, 3, 4)
const fBbFull = w.computeFingersForShape(['x', 1, 3, 3, 3, 1]);
check('A-образное полное баррэ Bb (x,1,3,3,3,1): пальцы x, 1, 2, 3, 4, 1', fBbFull, ['x', 1, 2, 3, 4, 1]);

// Форма [6, 5, 5, 3, 3, 3] не должна дублировать палец 4 на разных ладах
const f655333 = w.computeFingersForShape([6, 5, 5, 3, 3, 3]);
check('Форма 6,5,5,3,3,3 не содержит дублирования пальца 4 на разных ладах', f655333, [4, 3, 2, 1, 1, 1]);

// A-shape barre C: x, 3, 5, 5, 5, x -> пальцы x, 1, 3, 3, 3, x
const fCA = w.computeFingersForShape(['x', 3, 5, 5, 5, 'x']);
check('A-образная форма C (x,3,5,5,5,x): баррэ 3 пальцем', fCA, ['x', 1, 3, 3, 3, 'x']);
