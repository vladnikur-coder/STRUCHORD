...
// A-shape barre Bb: x, 1, 3, 3, 3, x -> 5-я струна 1 лад (палец 1), 4/3/2 струны 3 лад (пальцы 2, 3, 4)
const fBb = w.computeFingersForShape(['x', 1, 3, 3, 3, 'x']);
check('A-образная форма Bb (x,1,3,3,3,x): пальцы x, 1, 2, 3, 4, x', fBb, ['x', 1, 2, 3, 4, 'x']);
...
// A-shape barre C: x, 3, 5, 5, 5, x -> пальцы x, 1, 2, 3, 4, x
const fCA = w.computeFingersForShape(['x', 3, 5, 5, 5, 'x']);
check('A-образная форма C (x,3,5,5,5,x): пальцы x, 1, 2, 3, 4, x', fCA, ['x', 1, 2, 3, 4, 'x']);
