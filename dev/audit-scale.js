 ...
 *   svg-size    — размеры <svg> (атрибуты или inline style). Должны быть в rem.
...
const rules = [
  ...
  {
    cat: 'svg-size',
    // 0.256: размеры svg живут в inline style (WebKit отвергает rem в
    // ГЕОМЕТРИЧЕСКИХ атрибутах width/height). px в style — тот же грех.
    re: /<svg\b[^>]*?style="[^"]*?(?:width|height):\s*([0-9]*\.?[0-9]+)px/gi,
  },
  {
    cat: 'geometry',
