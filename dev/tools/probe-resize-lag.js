...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    const tick = (t) => { if (!window.__on) return; window.__frames.push(t - last); last = t; requestAnimationFrame(tick); };
  ...
  const r = await page.evaluate(() => { window.__on = false; const f = window.__frames.slice(5); f.sort((a, b) => a - b); return { n: f.length, p50: f[Math.floor(f.length * 0.5)].toFixed(1), p90: f[Math.floor(f.length * 0.9)].toFixed(1), p99: f[Math.floor(f.length * 0.99)].toFixed(1), max: f[f.length - 1].toFixed(1), over25: f.filter((v) => v > 25).length, longtasks: window.__lt }; });
